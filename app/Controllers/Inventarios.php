...
class Inventarios extends BaseController{
    ...
    public function generarReporteProblemas(){
        ...
        foreach($problemas as $key => $value){
            ...
            if ($value['Id_Tipo_Inspeccion'] == "0D32B331-76C3-11D3-82BF-00104BC75DC2" ||
                ...
                $pdf->SetFont('Arial','B',8); $pdf->Cell(9,4,utf8_decode($value['Aumento_Temperatura']."°C"),0,0,'R'); $pdf->Ln();
                $pdf->SetFont('Arial','',8); $pdf->Cell(58,4,utf8_decode('Temperatura estimada 50%:'),0,0,'L');
                $pdf->SetFont('Arial','B',8); $pdf->Cell(9,4,utf8_decode($carga_50."°C"),0,0,'R'); $pdf->Ln();
                $pdf->SetFont('Arial','',8); $pdf->Cell(58,4,utf8_decode('Temperatura estimada 100%:'),0,0,'L');
                $pdf->SetFont('Arial','B',8); $pdf->Cell(9,4,utf8_decode($carga_100."°C"),0,0,'R'); $pdf->Ln();
    ...
    public function generarReporteBaseLine(){
        ...
        foreach($baseLine as $key => $value){
            ...
            $data = array('Temp Amb' => $amb, 'Temp Máx' => $max, 'MTA' => $mta);
            $colors = array(
                'Temp Amb' => array(23,49,182),
                'Temp Máx' => array(245,0,0),
            ...
            $header = array('No. Inspección', 'Fecha Inspección','Estatus', 'Máx Temp. Admisible', 'Temp Máxima', 'Temp Ambiente','Notas');
            ...
            foreach($historialBaseLine as $keyBL => $valueBL){
                ...
                $pdf->SetFont('Arial','',8); $pdf->Cell($w[3],4,utf8_decode($valueBL['MTA']."°C"),0,0,'C');
                $pdf->SetFont('Arial','',8); $pdf->Cell($w[4],4,utf8_decode($valueBL['Temp_max']."°C"),0,0,'C');
                $pdf->SetFont('Arial','',8); $pdf->Cell($w[5],4,utf8_decode($valueBL['Temp_amb']."°C"),0,0,'C');
    ...
    public function generarResultadoDeAnalisis(){
        ...
        $pdf->MultiCell(0,9,utf8_decode("Informe de Servicio Análisis de Riesgos con\nTermografía Infrarroja"),0,'C');
        ...
        $pdf->MultiCell(0,5,utf8_decode("Por este medio, hacemos entrega de los resultados finales de la inspección por termografía infrarroja realizada en las instalaciones eléctricas y mecánicas de ".$detalle_ubicacion.", ubicadas en ".$Municipio.", ".$Estado.". Servicio realizado ".$strFecha."."),0,"J");
        $pdf->Ln(5);
        ...
        $pdf->SetFont('Arial','',11);
        $pdf->MultiCell(0,5,utf8_decode("Personal de ETIC, S.A. de C.V., se presentó en las instalaciones de ".$detalle_ubicacion." ubicadas en ".$Municipio.", ".$Estado.", ".$sitio.", ".$strFecha.", con el objeto de realizar la  Inspección por Termografía Infrarroja en los equipos seleccionados para su estudio definidos en conjunto con responsables de cada área, descargando la información recabada durante este servicio en el software ETIC System©."),0,"J");
        $pdf->Ln(15);
...
class PDF_RA extends FPDF{
    ...
    function Footer(){
        // if ( $this->PageNo() !== 1 ) {
            ...
            $this->Cell(85,4,utf8_decode("Especialistas en Termografía Industrial y Corporativa, S.A. de C.V."),0,0,"L"); $this->Ln();

            $this->Cell(85,4,utf8_decode("Sucursal matriz"),0,0,"L"); $this->Cell(0,4,utf8_decode("Sucursal Bajío"),0,0,"R"); $this->Ln();
            $this->Cell(85,4,utf8_decode("Col. Las Américas, C.P. 55076"),0,0,"L"); $this->Cell(0,4,utf8_decode("Col. El Dorado, C.P. 37590"),0,0,"R"); $this->Ln();
            $this->Cell(0,4,utf8_decode("Ecatepec de Morelos, Estado de México"),0,0,"L"); $this->Cell(0,4,utf8_decode("León, Guanajuato"),0,0,"R"); $this->Ln();
            $this->Cell(0,4,utf8_decode("Teléfonos: 55 8032 5401"),0,0,"L"); $this->Cell(0,4,utf8_decode("Teléfonos: 55 8032 5401"),0,0,"R"); $this->Ln();
