    ...
    tabla_Inspecciones = $("#TbInspecciones").DataTable({
        ...
        "columns": [
            ...
            { data: null, render: function ( data, type, row ) {
                    ...
                    // Si es la ultim inspeccion del sitio se puede seguir actualizando
                    if (row.ultimaInspeccionSitio == row.No_Inspeccion) {
                        opciones += `<button class="btn btn-sm Actualizar_Inspeccion_DB mr-1" style="background-color: #c09affed; color: #FFFFFF" value="${row.Id_Inspeccion}" title="Actualizar Inspección"><i class="fas fa-upload"></i></button>`;
                        // Si es la ultima inspeccion del sitio y esta cerrada se exporta a excel
                        if (row.Id_Status_Inspeccion == "73F27007-76B3-11D3-82BF-00104BC75DC2") {
                            opciones += `<button class="btn btn-success btn-sm" onclick="generarReporteListaProblemasExcel('${row.Id_Inspeccion}', '${row.Fecha_Inicio}', '${row.Fecha_Fin}')" title="Excel"><i class="fas fa-file-csv"></i></button>`;
                        }else{
                            // Si es la ultima inspeccion y sigue abierta se puede descargar y eliminar
                            opciones += `
                                <button class="btn btn-warning btn-sm Exportar_Inspeccion_DB mr-1" value="${row.Id_Inspeccion}" title="Exportar Inspección"><i class="fas fa-download"></i></button>
        ...
        columnDefs: [
            ...
            { 'targets': [ 17 ], className: 'dt-body-left' }
        ],
...
function generarReporteListaProblemasExcel(id_inspeccion, fecha_inicio, fecha_fin){
    let f1 = fecha_inicio.split(" ")[0]
    let f2 = fecha_fin.split(" ")[0]
    console.log('aquiiiii')
    window.location.href = `/inspecciones/generarReporteListaProblemasExcel/${id_inspeccion}/${f1}/${f2}`
    return;
}